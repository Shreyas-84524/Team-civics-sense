 ...
 */
export const verifyCitizenPhoneMsg91 = functions.https.onCall(async (data, context) => {
  // 1. Verify caller authentication
  if (!context.auth || !context.auth.uid) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'The caller must be authenticated to verify their phone number.'
    );
  }
  ...
  if (!phone || typeof phone !== 'string') {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'The function must be called with a valid "phone" parameter.'
    );
  }
  ...
  if (tenDigit.length !== 10 || !/^[6-9]/.test(tenDigit)) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'The phone number provided is not a valid 10-digit Indian mobile number.'
    );
  }
  ...
  if (msg91AuthKey && accessToken && typeof accessToken === 'string') {
    try {
      ...
      if (!response.ok || (result && result.type === 'error')) {
        ...
        throw new functions.https.HttpsError(
          'invalid-argument',
          'MSG91 OTP access token verification failed or expired.'
        );
      }
    } catch (fetchErr) {
      if (fetchErr instanceof functions.https.HttpsError) throw fetchErr;
      functions.logger.error(`[verifyCitizenPhoneMsg91] Server fetch error verifying token:`, fetchErr);
      throw new functions.https.HttpsError(
        'internal',
        'Failed to connect to MSG91 verification gateway.'
      );
    }
  } else if (msg91AuthKey && !accessToken) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Missing accessToken required for MSG91 server-side validation.'
    );
  }
  ...
  try {
    await db.runTransaction(async (transaction) => {
      ...
      const indexPayload: Record<string, any> = {
        uid: uid,
    ...
    throw new functions.https.HttpsError(
      'internal',
      'Failed to update citizen profile with verified phone.'
    );
  }
});
